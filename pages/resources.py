...
class AppVariables:
    ...
    app_vars = ['home', True, 11, ["home", "设置", "错别字", "聊天", "DNF-端游-韩翻中",  "DNF-端游-中翻韩", "DNF-手游", "冒险岛-手游", "黑色沙漠-端游", "黑色沙漠-手游", "TGF", "FM"], {"home": 0, "设置": 1, "错别字": 2, "聊天": 3, "DNF-端游-韩翻中": 4, "DNF-端游-中翻韩": 5, "DNF-手游": 6, "冒险岛-手游": 7, "黑色沙漠-端游": 8, "黑色沙漠-手游": 9, "TGF": 10, "FM": 11}]
...
class DefaultModelVariables:
    ...
    default_model_vars = ['ntRg23Av0MXK4P709Gj9V5U3XHmJgYx37991Oh9RUbM7neXT', 'gpt-4o', 'https://guoxh4o.openai.azure.com/', '94a744516fcc43ac90948e4d3927e6e0', 'guoxhgpt4o', '2023-05-15']
...
class AppConfig:
    ...
    app_config = ['', '', '', []]
    # 第一项为韩文，第二项为中文
    term_list = [ [] for _ in range(2) ]
    # 第一项为用韩文检索中文，第二项为用中文检索韩文
    term_dict = [ {} for _ in range(2) ]
...
class llmModel:
    ...
    # 0: openai_api_key, 1: openai_model_name, 2: azure_endpoint, 3: azure_api_key, 4: azure_model_name, 5: azure_api_version
    model_vars = ['ntRg23Av0MXK4P709Gj9V5U3XHmJgYx37991Oh9RUbM7neXT', 'gpt-4o', 'https://guoxh4o.openai.azure.com/', '94a744516fcc43ac90948e4d3927e6e0', 'guoxhgpt4o', '2023-05-15']

    openai_temperature = 0.1
    azure_temperature = 0.1

    translate_prompt_base = "请将以下中文内容翻译成韩文，内容简洁，不要有翻译腔："
    language_detection_prompt = """你是一个翻译助手，请根据用户输入，识别用户的源语言。仅返回用户的源语言，源语言与目标语言仅会从中文和韩文中出现，如果识别源语言不在中文和韩文范围内，返回0，示例：
